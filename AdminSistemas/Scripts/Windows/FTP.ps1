...
if (-not $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Host "[ERROR] Este script debe ejecutarse como Administrador" -ForegroundColor Magenta
    exit 1
}
...
# ============================================================================
function Print-Info   { param($msg) Write-Host "[INFO]  $msg" -ForegroundColor White }
function Print-Ok     { param($msg) Write-Host "[OK]    $msg" -ForegroundColor Magenta }
function Print-Error  { param($msg) Write-Host "[ERROR] $msg" -ForegroundColor DarkMagenta }
function Print-Warn   { param($msg) Write-Host "[WARN]  $msg" -ForegroundColor Pink }
...
function Cambiar-Grupo-Usuario {
    ...
    Write-Host ""
    Write-Host "  Nuevo grupo:" -ForegroundColor White
    Write-Host "  1) $GRUPO_REPROBADOS" -ForegroundColor Magenta
    Write-Host "  2) $GRUPO_RECURSADORES" -ForegroundColor Magenta
...
function Gestionar-Usuarios {
    ...
    Write-Host "  1) Crear nuevos usuarios" -ForegroundColor White
    Write-Host "  2) Cambiar grupo de un usuario" -ForegroundColor White
    Write-Host "  3) Eliminar usuario" -ForegroundColor White
    Write-Host "  4) Cambiar contrasena de usuario" -ForegroundColor White
    Write-Host "  5) Volver" -ForegroundColor White
    Write-Host ""
...
function Ver-Estado {
    ...
    if ($svc) {
        $color = if ($svc.Status -eq "Running") { "Magenta" } else { "DarkMagenta" }
    ...
    Write-Host "  Sitio IIS       : $estado" -ForegroundColor White
    ...
    Write-Host "  User Isolation  : $isoText" -ForegroundColor White
...
function Mostrar-Ayuda {
    Write-Host ""
    Write-Host "Uso: .\ftp_server.ps1 [opcion]" -ForegroundColor White
    Write-Host ""
    Write-Host "  -install   Instala y configura el servidor FTP (primera vez)" -ForegroundColor White
    Write-Host "  -users     Gestionar usuarios (crear, cambiar grupo, eliminar)" -ForegroundColor White
    Write-Host "  -status    Ver estado del servidor y usuarios" -ForegroundColor White
    Write-Host "  -restart   Reiniciar el servicio FTP" -ForegroundColor White
    Write-Host "  -verify    Verificar si IIS y FTP estan instalados" -ForegroundColor White
    Write-Host "  -list      Listar usuarios y estructura" -ForegroundColor White
    Write-Host "  -help      Mostrar esta ayuda" -ForegroundColor White
    Write-Host ""
    Write-Host "Orden recomendado (primera vez):" -ForegroundColor Magenta
    Write-Host "  1. .\ftp_server.ps1 -install" -ForegroundColor White
    Write-Host "  2. .\ftp_server.ps1 -users" -ForegroundColor White
    Write-Host ""
}
